...
function getShaHistory(sha) {
  return execAsync('git rev-list ' + sha + ' --all')
  .then(parseRevListOutput);
}

function getLastCommonSha(shas1, shas2) {
  if (!shas1.length || !shas2.length) {
    return null;
  }

  if (shas1[shas1.length - 1] !== shas2[shas2.length - 1]) {
    return null;
  }

  if (shas1.length === shas2.length && shas1[0] === shas2[0]) {
    return shas1[0];
  }

  var minLength = Math.min(shas1.length, shas2.length);

  for (var i = 1; i <= minLength; i++) {
    var sha1 = shas1[shas1.length - i];
    var sha2 = shas2[shas2.length - i];
    var match = sha1 === sha2;

    if (!match) {
      return shas1[shas1.length - i + 1];
    }
  }
...
var GitCommonAncestor = {
  ofShaAndBranch: function(headSha, baseBranch) {
    ...
    .then(function(history) {
      return getLastCommonSha(history[0], history[1]);
    });
  }
};
